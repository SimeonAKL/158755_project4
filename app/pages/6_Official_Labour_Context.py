...
def collapse_to_time_series(df, date_col, value_col, how="mean"):
    temp = df[[date_col, value_col]].copy()
    temp = temp.dropna(subset=[date_col, value_col])

    if how == "sum":
        temp = temp.groupby(date_col, as_index=False)[value_col].sum()
    else:
        temp = temp.groupby(date_col, as_index=False)[value_col].mean()
...
for df_, col_ in [
    (df_jobs, jobs_date_col),
    (df_monthly, monthly_date_col),
...
with st.container(border=True):
    ...
    with cb:
        ...
        d2a = {
            INDICATOR_LABELS.get(c, c.replace("_", " ").title()): c
            for c in numeric_cols(df_ctx)
        }
...
# Aggregate to one value per date before plotting
df_ind = collapse_to_time_series(df_ctx, ctx_dc, sel_ind, how="mean")
...
df_jp = collapse_to_time_series(df_jobs, jobs_date_col, "totals", how="mean")
df_cp = collapse_to_time_series(df_ctx, ctx_dc, sel_ind, how="mean")

fig2 = go.Figure()

fig2.add_trace(
    go.Scatter(
        x=df_jp[jobs_date_col],
        y=df_jp["totals"],
...
fig2.add_trace(
    go.Scatter(
        ...
        hovertemplate=f"<b>{sel_disp}</b>: %{{y:.2f}}<extra></extra>",
        yaxis="y2",
    )
)

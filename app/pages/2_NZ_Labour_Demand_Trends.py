...
with cb:
    skill_view = st.radio("Skill view", ["Both", "Skilled only", "Unskilled only"])
...
with c1: st.metric("Current Demand",   f"{latest_val:.1f}", f"{delta:+.1f} vs prior month")
with c2: st.metric("Period Peak",      f"{peak_val:.1f}",   peak_dt.strftime("%b %Y"))
with c3: st.metric("Period Trough",    f"{trough_val:.1f}", trough_dt.strftime("%b %Y"))
with c4: st.metric("vs Period Average", f"{avg_val:.1f}",   f"{latest_val - avg_val:+.1f}")
...
fig1 = go.Figure()
fig1.add_trace(
    line_trace(df_f[date_col], df_f[total_col],
               "Hiring Demand Index", color=HISTORICAL_COLOR, width=2)
)
fig1.add_hline(y=avg_val, line_dash="dot", line_color=C["border"], line_width=1.5,
               annotation_text=f"Period avg: {avg_val:.1f}",
               annotation_position="bottom right",
               annotation_font=dict(size=10, color=C["text_secondary"]))
fig1.update_layout(**plotly_layout(
    "Overall NZ Hiring Demand Trend", x_label="", y_label="Hiring Demand Index"))
...
fig2.add_trace(go.Bar(
    x=ann_avg["year"].astype(str), y=ann_avg["pct"],
...
if skill_view in ["Both", "Skilled only"]:
    fig3.add_trace(
        line_trace(df_f[date_col], df_f[skilled_col],
...
st.download_button(
    ...
    data=df_f[[date_col, total_col, skilled_col, unskilled_col]].to_csv(index=False).encode("utf-8"),

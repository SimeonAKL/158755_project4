...
# ── Hero ──────────────────────────────────────────────────────────────────────
page_hero(
    title="NZ Labour Demand Forecaster",
    subtitle=(
        "A decision-support view of short-term hiring demand across New Zealand — "
        "tracking where hiring is rising, softening, or holding steady across "
        "regions, sectors, and occupations."
    ),
    eyebrow="New Zealand  ·  Labour Market Intelligence",
)
...
df_jobs = (df_jobs.sort_values("date")
                  .dropna(subset=["date", "totals"])
                  .reset_index(drop=True))

df_totals_fc["date"] = pd.to_datetime(df_totals_fc["date"], errors="coerce")
df_totals_fc = (df_totals_fc.sort_values("date")
                             .dropna(subset=["date", "forecast"])
                             .reset_index(drop=True))
...
with col1:
    st.metric("Current Hiring Demand",
              f"{latest_jobs_value:.1f}",
              latest_jobs_date.strftime("%b %Y"))
with col2:
    st.metric("Historical Peak",
              f"{peak_jobs_value:.1f}",
              peak_jobs_date.strftime("%b %Y"))
with col3:
    st.metric("Historical Trough",
              f"{trough_jobs_value:.1f}",
              trough_jobs_date.strftime("%b %Y"))
with col4:
    st.metric("Near-Term Forecast",
              f"{latest_fc_value:.2f}",
              f"{fc_change:+.2f} vs prior period")

st.info(
    "Short-term hiring demand remains active nationally, "
    "but the picture is uneven across sectors and regions."
)
...
st.write(
    "New Zealand hiring demand has moved through clear cycles of contraction and "
    "recovery. The series responds to broad economic conditions, making timely "
    "monitoring valuable for workforce planning and market positioning."
)
...
st.write(
    ...
    "not a long-run projection — designed to identify whether hiring momentum is "
...
with col_left:
    ...
    for label, col in industry_map.items():
        if col in df_ind_fc.columns:
            s = df_ind_fc[col].dropna()
            if not s.empty:
                industry_latest[label] = s.iloc[-1]

    if industry_latest:
        ind_df = (
            pd.DataFrame({
                "Sector": list(industry_latest.keys()),
        ...
        st.caption(
            f"**{top_ind['Sector']}** leads the sector forecasts "
            f"at **{top_ind['Forecast Index']:.2f}**."
        )
...
with col_right:
    ...
    for label, col in region_map.items():
        if col in df_reg_fc.columns:
            s = df_reg_fc[col].dropna()
            if not s.empty:
                region_latest[label] = s.iloc[-1]
    ...
    if region_latest:
        ...
        st.caption(
            f"**{top_reg['Region']}** records the highest regional forecast "
            f"at **{top_reg['Forecast Index']:.2f}**."
        )
...
with c2:
    info_card(
        ...
        "forecasts — drawn from MBIE Jobs Online data and Stats NZ official indicators.",
        "📊",
    )
with c3:
    info_card(
